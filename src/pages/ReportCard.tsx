        ...
        {REPORT_CARD.map((s, i) => (
          <Card key={i}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              ...
              <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '10px', color: s.trend === 'up' ? C.teal : s.trend === 'down' ? C.red : C.muted }}>{s.trend === 'up' ? '↑' : s.trend === 'down' ? '↓' : '—'}</span>
            </div>
            <div style={{ display: 'flex', gap: '3px', marginBottom: '8px' }}>
              {Array.from({ length: 10 }, (_, j) => <div key={j} style={{ flex: 1, height: '5px', borderRadius: '2px', background: j < s.score ? C.accent : C.s3 }} />)}
            </div>
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '26px', fontWeight: 800, color: s.score >= 7 ? C.accent : s.score >= 5 ? C.amber : C.red, marginBottom: '6px' }}>
