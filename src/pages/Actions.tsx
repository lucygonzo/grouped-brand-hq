    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
        {(['Now', 'Next', 'Later'] as const).map(g => (
          <div key={g}>
            <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '9px', color: g === 'Now' ? C.accent : g === 'Next' ? C.amber : C.muted, textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: '10px' }}>{g}</div>
            {ACTIONS.filter(a => a.status === g).map((a, i) => (
              <Card key={i} style={{ marginBottom: '8px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '9px', color: C.muted }}>{a.owner}</span>
