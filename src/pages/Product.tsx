      ...
      {PRODUCT_ROADMAP.map((r, i) => (
        <div key={i} style={{ display: 'flex', gap: '14px', marginBottom: '10px' }}>
          <div style={{ width: '90px', flexShrink: 0, paddingTop: '14px' }}>
            <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '9px', color: C.accent, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{r.phase}</div>
            <div style={{ fontFamily: 'Inter, sans-serif', fontSize: '10px', color: r.status === 'Live' ? C.teal : r.status.includes('2026') ? C.amber : C.sub, marginTop: '2px' }}>{r.status}</div>
          </div>
          <Card style={{ flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '14px', fontWeight: 700, color: C.text }}>{r.label}</div>
              <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '9px', color: C.muted }}>{r.rev}</div>
            </div>
