        ...
        <div>
          ...
          <Card style={{ marginBottom: '14px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', margin: '10px 0 8px' }}>
              {VERBAL_IDENTITY.brandAnchorCopy.lines.map((l, i) => (
                <div key={i} style={{ background: C.accentGlow, border: `1px solid ${C.accent}30`, borderRadius: '6px', padding: '8px 14px', fontFamily: "'Satoshi', sans-serif", fontWeight: 700, fontSize: '14px', color: C.accent }}>{l}</div>
              ))}
            </div>
          ...
          <Card style={{ marginTop: '14px' }}>
            ...
            {VERBAL_IDENTITY.taglines.map((t, i) => (
              <div key={i} style={{ fontFamily: "'Satoshi', sans-serif", fontSize: '13px', color: C.text, padding: '6px 0', borderBottom: i < VERBAL_IDENTITY.taglines.length - 1 ? `1px solid ${C.border}` : 'none', fontStyle: 'italic' }}>"{t}"</div>
            ))}
          </Card>
          <Card style={{ marginTop: '14px' }}>
            ...
            {VERBAL_IDENTITY.proofReframe.map((r, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: '10px', padding: '6px 0', borderBottom: i < VERBAL_IDENTITY.proofReframe.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: '12px', color: C.muted, textDecoration: 'line-through' }}>{r.proxy}</span>
                <span style={{ color: C.accent, fontSize: '10px' }}>→</span>
                <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: '12px', color: C.text }}>{r.real}</span>
              </div>
            ))}
          </Card>
        </div>
            ...
            {VERBAL_IDENTITY.voiceSpectrum.map((s, i) => (
              <div key={i} style={{ marginBottom: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '10px', color: C.text }}>{s.dim}</span>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '9px', color: C.muted }}>{s.pct}%</span>
                </div>
                ...
                <div style={{ fontFamily: "'Satoshi', sans-serif", fontSize: '11px', color: C.muted, marginTop: '3px' }}>{s.note}</div>
              </div>
            ))}
          </Card>
          <Card>
            ...
            {VERBAL_IDENTITY.voiceTraits.map((v, i) => (
              <div key={i} style={{ padding: '10px 0', borderBottom: i < VERBAL_IDENTITY.voiceTraits.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '9px', color: C.accent, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '4px' }}>{v.trait}</div>
          ...
          {VERBAL_IDENTITY.segments.map((seg, i) => (
            <Card key={i} style={{ marginBottom: '14px' }}>
              <div style={{ marginBottom: '14px' }}>
                <div style={{ fontFamily: "'Satoshi', sans-serif", fontSize: '15px', fontWeight: 700, color: C.text }}>{seg.name}</div>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '9px', color: C.muted, marginTop: '2px' }}>{seg.range}</div>
              </div>
              <div style={{ fontFamily: "'Satoshi', sans-serif", fontSize: '12px', color: C.text, fontStyle: 'italic', background: C.s3, padding: '10px', borderRadius: '6px', marginBottom: '12px', lineHeight: 1.65 }}>{seg.story}</div>
              ...
              <div style={{ background: C.accentGlow, borderLeft: `2px solid ${C.accent}`, borderRadius: '0 4px 4px 0', padding: '7px 12px' }}>
                ...
                <span style={{ fontFamily: "'Satoshi', sans-serif", fontSize: '12px', color: C.text }}>{seg.cta}</span>
              </div>
            </Card>
          ))}
        </div>
